...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  uint16_t index = 0;
  char ver=c;
  if (c >= 'A' && c <= 'Z'){index = (c - 'A' + 11) * 8; // Para letras maiúsculas
  }else  if (c >= '0' && c <= '9'){index = (c - '0' + 1) * 8; // Adiciona o deslocamento necessário
  }else if (c >= 'a' && c<= 'z'){index = (c - 'a' + 37)* 8; // Adiciona deslocamento para letras minusculas
